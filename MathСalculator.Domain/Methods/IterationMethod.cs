...
namespace MathСalculator.Domain.Methods
{
    public class SimpleIterationMethod
    {
        ...
        /// <summary>
        /// Квазиньютоновский итерационный метод
        /// </summary>
        /// <param name="Function"></param>
        /// <param name="X0">начальное приближение</param>
        /// <param name="Epsilon"></param>
        /// <returns></returns>
        public double Quasi_NewtonMethod(string Function, double X0, double Epsilon)
        {
            double bet = 0.01;
            double betLast = 0;
            double gam = 0.0001;
            double y = X0 - bet * CalcMethods.ParseFunction(Function, X0) / CalcMethods.DerivativeOfFunction(Function, X0);
            double k = 1;
            ...
            while ((Math.Abs(y - X0) > Epsilon))
            {
                ...
                k++;
                resultTEXT += "\n k =" + k + "\n";
                bet = Math.Abs(CalcMethods.ParseFunction(Function, X0)) * gam / Math.Abs(CalcMethods.ParseFunction(Function, y)) * bet;
                gam = gam * Math.Abs(CalcMethods.ParseFunction(Function, X0)) * bet / Math.Abs(CalcMethods.ParseFunction(Function, y)) * betLast;
                X0 = y;
                y = X0 - CalcMethods.ParseFunction(Function, X0) / CalcMethods.DerivativeOfFunction(Function, X0);
            }
            resultTEXT += "Результат =" + X0 + "\n";
